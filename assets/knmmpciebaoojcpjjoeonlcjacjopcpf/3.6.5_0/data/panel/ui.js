...
const apiEndpoint = "https://app-how-to-use-it.com/NewVpn.json";
//// Sticky
...
function showMain() {
  ...
  for (let i in app.conf.data.servers) {
    ...
    for (let j in app.conf.data.servers[i].proxies) {
      ...
      flagDescr.textContent =
        app.conf.data.servers[i].proxies[j].country +
        "  " +
        app.conf.data.servers[i].proxies[j].proxy.split(":")[0];
