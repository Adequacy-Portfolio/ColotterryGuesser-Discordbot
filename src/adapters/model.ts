//@ts-nocheck
export function byPositionReport(data: any) {
...
export function bySequenceReport(data: any) {
...
export function byPatternReport(data: any) {
  const sorted_stats = Object.entries(data)
    .sort((first, second) => {
      if (first[1] < second[1]) {
        return +1;
...
export function last10DrawsReport(data: any) {
  return `
